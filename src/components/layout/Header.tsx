...
export function Header() {
  ...
  return (
    <>
      <header
        className={`sticky top-0 z-50 border-b px-5 backdrop-blur-xl transition-all duration-300 ${
          hasScrolled
            ? "border-rose-200/80 bg-white/90 shadow-lg shadow-rose-200/35 dark:border-stone-800 dark:bg-stone-950/90 dark:shadow-black/25"
            : "border-rose-200/70 bg-white/80 dark:border-stone-800 dark:bg-stone-950/80"
        }`}
      >
        <div
          className={`mx-auto flex max-w-6xl items-center justify-between gap-5 transition-all duration-300 ${
            hasScrolled ? "h-16" : "h-18.25"
          }`}
        >
          <Link
            href="/"
                ...
                <Link
                  ...
                  className={`focus-ring relative rounded-full px-4 py-2 text-sm font-semibold transition ${
                    isActive
                      ...
                      : "text-stone-600 hover:bg-rose-50 hover:text-rose-700 dark:text-stone-300 dark:hover:bg-stone-900 dark:hover:text-rose-200"
                  }`}
                >
                  {item.label}
