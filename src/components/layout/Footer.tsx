      ...
      <Reveal>
        <div className="mx-auto grid max-w-6xl gap-10 md:grid-cols-[1.2fr_0.8fr_0.8fr]">
          <div>
            <Link
              href="/"
              className="text-xl font-semibold tracking-tight text-stone-950 transition hover:text-rose-700 dark:text-rose-50 dark:hover:text-rose-200"
            >
              {content.brand}
            </Link>
            ...
            <nav className="mt-5 grid gap-3">
              {content.nav.map((item) => (
                <Link
                  key={item.href}
                  href={item.href}
                  className="interactive-press w-fit transition hover:translate-x-1 hover:text-rose-700 dark:hover:text-rose-200"
                >
                  {item.label}
                </Link>
              ))}
        ...
        <div className="mx-auto mt-10 flex max-w-6xl flex-col gap-3 border-t border-rose-200/70 pt-6 text-xs text-stone-500 dark:border-stone-800 dark:text-stone-500 md:flex-row md:items-center md:justify-between">
          ...
          <p>Natural beauty, refined with care.</p>
        </div>
        ...
        <div className="mx-auto mt-6 max-w-6xl border-t border-rose-200/70 pt-6 text-center text-xs text-stone-500 dark:border-stone-800 dark:text-stone-500">
          <p>
            {language === "pl"
              ? "Strona wykonana przez Piotra Zandeckiego."
              : "Website created by Piotr Zandecki."}
          </p>
        </div>
      </Reveal>
    </footer>
